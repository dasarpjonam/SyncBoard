...
interface WorkspaceContextProps {
  ...
  completeSetup: (newConfig: WorkspaceConfig) => Promise<void>;
}
...
export const WorkspaceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadWorkspace = async (path: string) => {
    try {
      ...
      if (configContent) {
        ...
        // Flag for setup screen instead of auto-creating defaults
  ...
  const completeSetup = async (newConfig: WorkspaceConfig) => {
    if (!workspacePath) return;
    await window.electronAPI.writeFile(`${workspacePath}/config.yaml`, yaml.dump(newConfig));
    setConfig(newConfig);
    setIsSetupPending(false);
  };
  ...
  return (
    <WorkspaceContext.Provider value={{
      workspacePath, items, config, apiKey, isSetupPending,
      setWorkspacePath: setWorkspacePathPersist, setItems, addItem, updateItem,
      setConfig, setApiKey, saveConfig, loadWorkspace, completeSetup
    }}>
      {children}
